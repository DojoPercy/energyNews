...
const NewsUpdates = ({ news }) => {
  ...
  return (
    <div>
      <div className="p-0 grid grid-cols-2 lg:grid-cols-3 md:grid-cols-3 space-x-0 lg:space-y-0 space-y-0">
        {news.map((article) => (
          <div key={article.id} className="group bg-white  rounded-lg pl-4 py-4 ">
            ...
            <h4 className="text-lg font-normal mt-2">
              <a href={`/news/${encodeURIComponent(article.title)}`} className=" line-clamp-4">
                {article.title}
              </a>
            </h4>
          </div>
